 ...
 *
 * <p>운영에서 허용하는 조합은 <b>gcs/s3 + 비어 있지 않은 버킷</b>(업로드 켬) 또는
 ...
 * 버킷 생성·권한·워크플로 두 줄 추가 순서는 README "업로드 저장소 (운영)" 에 있다.
 *
 * <p>버킷이 빈 경우는 보통 이 빈보다 먼저 만들어지는 {@link GcsStorageService} 또는 {@link S3StorageService} 생성자가 먼저 잡는다
...
public class StorageConfigValidator {
    ...
    static final String WORKFLOW_ENV_VARS = "배포 환경변수";

    /** 업로드를 꺼 둔 채 뜰 때 남기는 경고 — 켜는 방법을 그대로 담는다. */
    static final String DISABLED_WARNING = "운영 사진 업로드가 꺼져 있습니다 — S3 또는 GCS 준비 후 "
            + WORKFLOW_ENV_VARS + " 에 STORAGE_TYPE=s3/S3_BUCKET 또는 STORAGE_TYPE=gcs/GCS_BUCKET 을 넣으세요.";

    /** 운영에서 허용하는 두 상태. */
    enum Mode {
        /** GCS 또는 S3 버킷에 저장한다 — 업로드가 켜져 있다. */
        REMOTE,
    ...
    public StorageConfigValidator(BookeyProperties properties) {
        if (validate(properties.storage()) == Mode.DISABLED) {
            ...
            String type = properties.storage().type();
            if ("s3".equalsIgnoreCase(type)) {
                log.info("운영 업로드 저장소 확인: s3://{}", properties.storage().s3().bucket());
            } else {
    ...
    /**
     * 운영에서 허용하는 조합은 type=gcs/s3 + 비어 있지 않은 버킷, 또는 type=none 이다.
    ...
    static Mode validate(BookeyProperties.Storage storage) {
        ...
        }
        {
            throw new IllegalStateException(
                    "운영 프로파일에서는 bookey.storage.type 이 gcs/s3(업로드 켬) 또는 none(업로드 끔) 이어야 합니다(현재: "
                            + (type == null || type.isBlank() ? "(비어 있음)" : type)
                            + "). 로컬 디스크로 뜨면 인스턴스가 교체될 때마다 업로드한 사진이 사라집니다. "
                            + WORKFLOW_ENV_VARS + " 에 STORAGE_TYPE=s3 와 S3_BUCKET 을 넣거나, "
        ...
        if (bucket == null || bucket.isBlank()) {
            throw new IllegalStateException(
                    "bookey.storage.gcs.bucket 이 비어 있습니다. 환경변수 GCS_BUCKET=<버킷 이름> 이 필요합니다 — 운영이면 "
                            + WORKFLOW_ENV_VARS + " 에 GCS_BUCKET 을 등록하세요(README '업로드 저장소 (운영)' 순서대로).");
        }
        return bucket;
    }

    static BookeyProperties.Storage.S3 requireS3(BookeyProperties.Storage storage) {
        BookeyProperties.Storage.S3 s3 = storage == null ? null : storage.s3();
        String bucket = s3 == null ? null : s3.bucket();
        if (bucket == null || bucket.isBlank()) {
            throw new IllegalStateException(
                    "bookey.storage.s3.bucket 이 비어 있습니다. 환경변수 S3_BUCKET=<버킷 이름> 이 필요합니다 — 운영이면 "
                            + WORKFLOW_ENV_VARS + " 에 STORAGE_TYPE=s3 와 S3_BUCKET 을 등록하세요(README '업로드 저장소 (운영)' 순서대로).");
        }
        return s3;
    }
}
